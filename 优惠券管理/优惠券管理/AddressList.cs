...
namespace 优惠券管理
{
    public partial class AddressList : Form
    {
        ...
        {
           IEnumerable< Address > datas=new HHDapperSql().Query<Address>("select * from Address order by Id desc");
            dataGridView1.DataSource = datas;
        }

        private void 新增ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            var f=new AddressEdit();
        ...
        private void 删除选中ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("确定删除选中行吗？", "删除", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                ...
                var sql = "delete from Address where Id in (" + inid + ")";
